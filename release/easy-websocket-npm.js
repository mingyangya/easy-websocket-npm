        ...
        _createClass(EasyWebSocket, [{
            ...
            value: function connect(data) {
                ...
                this.ws.onmessage = function (evet) {
                    var pingData = void 0;
                    if (_this2.opt.serverType === "deamon") {
        ...
        _createClass(EasyWebSocket, [{
            ...
            value: function connect(data) {
                ...
                this.ws.onopen = function (e) {
                    ...
                    if (_this6.opt.cmd) {
                        //是否开启心跳检测
                        _this6.heartCheck(_this6.opt.cmd);
                    }
                    if (_this6.opt.debug) {
                        console.log(_this6.opt.name + "\u8FDE\u63A5\u6210\u529F", e);
                    }
                    //连接成功
                    _this6.opt.success && _this6.opt.success(_this6.opt.index);

                    if (data !== undefined) {
                        _this6.sendMessage(data);
                    }
                };
                this.ws.onmessage = function (evet) {
                    var pingData = void 0;
                    if (_this6.opt.serverType === "deamon") {
                        pingData = JSON.parse(evet.data).message;
                    } else {
                        pingData = evet.data;
                    }
                    if (pingData === "pong") {
                        _this6.pingPong = 'pong'; // 服务器端返回pong,修改pingPong的状态
                    }
                    return _this6.opt.msgCb(evet);
                };
                this.ws.onerror = function (e) {
                    _this6.errorHandle(e);
                };
                this.ws.onclose = function (err) {
                    _this6.closeHandle(err);
                };
            }
        }, {
            key: "sendMessage",
            ...
            value: function closeHandle() {
                ...
                if (this.status !== 'close') {
                    if (this.connectNum < this.opt.failNum) {
                        setTimeout(function () {
                            if (_this7.pingInterval !== undefined && _this7.pongInterval !== undefined) {
                                // 清除定时器
                                clearInterval(_this7.pingInterval);
                                clearInterval(_this7.pongInterval);
                            }
                            _this7.connect(); // 重连
                        }, this.opt.delayConnectTime);
                    } else {
            ...
            value: function heartCheck(cmd) {
                ...
                this.pingPong = 'ping'; // ws的心跳机制状态值
                this.pingInterval = setInterval(function () {
                    if (_this8.ws.readyState === 1) {
                        // 检查ws为链接状态 才可发送
                        if (typeof cmd !== "string") {
                            cmd = JSON.stringify(cmd);
                        }
                        _this8.ws.send(cmd); // 客户端发送ping
                    }
                }, this.opt.pingTime);

                this.pongInterval = setInterval(function () {
                    if (_this8.pingPong === 'ping') {
                        _this8.closeHandle('pingPong没有改变为pong'); // 没有返回pong 重启webSocket
                    } else {
                        if (_this8.opt.debug) {
                            console.log('返回pong');
                        }
                        _this8.pingPong = 'ping'; // 重置为ping 若下一次 ping 发送失败 或者pong返回失败(pingPong不会改成pong)，将重启.
                    }
                }, this.opt.pongTime);
            }
        }]);
